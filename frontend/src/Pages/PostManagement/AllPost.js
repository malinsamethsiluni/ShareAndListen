                  ...
                  <div className="user-details-card">
                    <div className="name-section-post">
                      ...
                      {post.userID !== loggedInUserID && (
                        <button
                          className={`follow-button ${followedUsers.includes(post.userID) ? 'unfollow' : 'follow'}`}
                    ...
                    {post.userID === loggedInUserID && (
                      <div className="action-buttons">
                        <FaEdit
                          onClick={() => handleUpdate(post.id)} className="action-btn-icon" />
                        <RiDeleteBin6Fill
                    ...
                    {post.comments?.map((comment) => (
                      <div key={comment.id} className="comment-card">
                        <div className="comment-details">
                          <p className="comment-username">{comment.userFullName}</p>
                        ...
                        <div className="comment-action-buttons">
                          {comment.userID === loggedInUserID && (
                            <>
                              {editingComment.id === comment.id ? (
                                <>
                                  <FiSave className="comment-btn"
                                    onClick={() =>
                                      handleSaveComment(post.id, comment.id, editingComment.content)
                                    } />
                                  <TbPencilCancel className="comment-btn"
                                    onClick={() => setEditingComment({})} />
                                </>
                              ) : (
                                <>
                                  <GrUpdate className="comment-btn" onClick={() =>
                                    setEditingComment({ id: comment.id, content: comment.content })
                                  } />
                                  <MdDelete className="comment-btn" onClick={() => handleDeleteComment(post.id, comment.id)} />
                                </>
                              )}
                            </>
                          )}
